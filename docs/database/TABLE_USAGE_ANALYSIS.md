...
### ❌ 使用されていない可能性が高いテーブル（1個）

| テーブル名 | 定義場所 | 使用状況 | 推奨アクション |
|-----------|---------|---------|---------------|
| `admins` | `src-tauri/src/database/mod.rs:216` | **未使用**（SELECT/INSERT/UPDATE/DELETEの使用箇所が見つからない） | **削除を検討** |
...
- **総テーブル数**: 21個（`organization_master`削除後）
- **使用中**: 19個
- **未使用**: 1個（`admins`）
- **削除済み**: 1個（`organization_master`）
