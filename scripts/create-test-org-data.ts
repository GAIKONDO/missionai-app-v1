...
import { createOrg, deleteOrg, getOrgTreeFromDb } from '../lib/orgApi';
 ...
 * テスト組織を作成
 */
async function createTestOrgs(): Promise<void> {
  try {
    ...
    if (existingIds.length > 0) {
      ...
      console.log('\n先に削除してください: npm run ts-node scripts/create-test-org-data.ts delete\n');
      return;
    }
    ...
    console.log('npm run ts-node scripts/create-test-org-data.ts delete\n');
  } catch (error: any) {
 ...
 * テスト組織を削除
 */
async function deleteTestOrgs(): Promise<void> {
  try {
 ...
 * テスト組織の一覧を表示
 */
async function listTestOrgs(): Promise<void> {
  try {
    ...
    if (orgIds.length === 0) {
      ...
      console.log('作成するには: npm run ts-node scripts/create-test-org-data.ts create\n');
      return;
    }
    ...
    console.log('削除するには: npm run ts-node scripts/create-test-org-data.ts delete\n');
  } catch (error: any) {
      ...
      try {
        switch (command) {
          ...
          default:
            ...
            console.log('使用可能なコマンド: create, delete, list');
            process.exit(1);
        }
      } catch (error: any) {
