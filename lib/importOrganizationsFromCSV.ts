...
async function readCSVFile(filePath: string): Promise<string[][]> {
  try {
    ...
    // 絶対パスでない場合、プロジェクトルートからの相対パスとして扱う
    if (!filePath.startsWith('/') && !filePath.match(/^[A-Z]:/)) {
      // プロジェクトルートのパスを取得
      const projectRoot = await callTauriCommand('get_project_root', {}) as string;
      
      // パスを結合（path.joinの代わりに手動で結合）
      // 先頭の./を削除
      const relativePath = filePath.replace(/^\.\//, '');
      actualPath = `${projectRoot}/${relativePath}`;
    }
    ...
    if (!result || !result.success || !result.data) {
      ...
      // より詳細なエラーメッセージを提供
      throw new Error(`${errorMsg}\nファイルパス: ${actualPath}\nヒント: 絶対パスを指定するか、プロジェクトルートからの相対パスを確認してください。`);
    }
 ...
 */
export async function importOrganizationsFromCSV(csvPath?: string): Promise<void> {
  try {
